...
let planks = [
  ...
  [00, 00, 00, 00, 00, 00, 00, 00, 00],
  [00, 00, 00, 00, 00, 00, 00, 00, 00],
  [C1, A2, B1, D1, C2, D2, C1, B2, A2],
  [B1, D2, C2, A2, B2, B1, A1, C1, A1],
  [A1, C1, B2, D2, C1, A1, C2, D1, B1],
  [D1, A2, C1, A1, D1, C1, B1, C2],
  [B2, C2, D1, B1, C2, B2, A2, D2, B2],
  [B1, A1, B2, A2, D2, A1, C2, B2, A2],
  [D2, C1, D1, B2, C1, D2, A1, C2, D2],
  [B2, A2, C2, A1, B1, C1, D1, A1, B2],
  [C1, D1, B1, D2, C2, D1, A2, B1, C1],
  [A1, C1, A2, B2, A1, C2, D2, D1],
  [C2, B2, D1, C1, D2, B1, A1, D1],
  [A2, C2, B2, A1, C1, D1, B1, C2],
  [D1, B1, A2, D2, B2, A1, C1, D2, B2],
  [B2, C1, D1, C2, A2, D2, B1, A2, C1],
  [D2, A1, B2, D1, A1, C2, D2]
];
let obsticles = [
  {
    description: "pantry",
...
window.onload = () => {
  ...
  // Normalize offsets
  offsets = offsets.map(element =>
    Math.floor((element / (47 + 5 / 8)) * scale)
  );
  generatePlanks();
  resize();
};
...
function draw() {
  for (let row = 0; row < planks.length; row++) {
    ...
    ctx.fillText(
      (offsets[row] * (47 + 5 / 8)) / scale,
      7,
      row * height + 0.12 * scale
    );
  }
  ...
  (ctx.fillStyle = "#222222"),
    obsticles.forEach(obsticle => {
      ctx.fillRect(
        Math.floor((obsticle.x / (47 + 5 / 8)) * scale),
        Math.floor((obsticle.y / (47 + 5 / 8)) * scale),
        Math.floor((obsticle.width / (47 + 5 / 8)) * scale),
        Math.floor((obsticle.height / (47 + 5 / 8)) * scale)
      );
    });
  ...
  ctx.fillRect(
    0,
    Math.floor((room.height / (47 + 5 / 8)) * scale),
    ctx.canvas.width,
    ctx.canvas.height - Math.floor((room.height / (47 + 5 / 8)) * scale)
  );
  ctx.fillRect(
    Math.floor((room.width / (47 + 5 / 8)) * scale),
    0,
    ctx.canvas.width - Math.floor((room.width / (47 + 5 / 8)) * scale),
    Math.floor((room.height / (47 + 5 / 8)) * scale)
  );
}
